...
The ORM uses the **same filter and param shape** for both MySQL and Mongo. Use this page as the single reference for all CRUD operations.
...
2. **Condition dict** — Use **op** and **value** (and optionally **bool**):
   ...
   - **op** — Operator: `=`, `!=`, `>`, `<`, `<=`, `>=`. Mongo also supports `in`, `nin`.
...
filters={'role': {'op': 'in', 'value': ['admin', 'user']}}   # Mongo (and MySQL if supported)
```
