...
Use **update(filters, data, params)** to update one or many rows/documents. The same API works for MySQL and Mongo.
...
- **data** — Dict of field names and new values. **Must be a dict**; the ORM applies this as the update payload (e.g. MySQL SET clause, Mongo `$set`).
