...
**Update** behaves as “set these fields” (partial update): MySQL `SET col = val`, Mongo `$set`. It does **not** replace the entire row/document in one call. To replace the full structure, you can:
...
DQuode does **not** run or ship migrations. Tables and collections must be created and altered outside the app (manual SQL, migration tools, or Mongo shell). Constraints (unique, foreign key, indexes) are managed in the database; the ORM does not enforce them beyond what the engine does.
